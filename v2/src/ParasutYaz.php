...
final class ParasutYaz
{
    ...
    /**
     * fable-091: ÇOK KALEMLİ ekstra fatura gövdesi (aylık müşterinin ay içi ekstraları).
     * sabitGovde'nin çok kalemli hâli — her kalemin kendi adedi, birimi ve KDV'si vardır.
     * @param list<array{ad:string,adet:float,birim:float,kdv:float,urun_id:string}> $kalemler
     */
    public function ekstraGovde(string $contactId, string $son, array $kalemler, int $vadeGun): array
    {
        $vade = date('Y-m-d', strtotime($son . ' +' . max(0, $vadeGun) . ' day'));
        $details = [];
        foreach ($kalemler as $k) {
            $d = [
                'type'       => 'sales_invoice_details',
                'attributes' => [
                    'quantity'    => (float) $k['adet'],
                    'unit_price'  => (float) $k['birim'],
                    'vat_rate'    => (float) $k['kdv'],
                    'description' => (string) $k['ad'],
                ],
            ];
            // Paraşüt kalemsiz fatura kabul etmiyor (422 "Ürün/hizmet doldurulmalı").
            if ((string) $k['urun_id'] !== '') {
                $d['relationships'] = ['product' => ['data' => ['id' => (string) $k['urun_id'], 'type' => 'products']]];
            }
            $details[] = $d;
        }
        return ['data' => [
            ...
            'attributes' => [
                ...
                'issue_date'  => $son,
                'due_date'    => $vade,
                'description' => date('m.Y', strtotime($son)) . ' dönemi ek hizmet bedelleri',
                'print_note'  => $this->faturaNotu(),
            ],
            'relationships' => [
                'contact' => ['data' => ['id' => $contactId, 'type' => 'contacts']],
    ...
    /**
     * 🔒 EKSTRA KALEM FATURASI (fable-091, Ömer 28 Ağu: "irsaliyesiz olan faturalara ekstra
     * eklenecek alan kurgula, ay içinde ekstralara yazayım").
     *
     * Aylık müşterinin o aya girilmiş, HENÜZ FATURALANMAMIŞ ekstra kalemleri TEK faturada
     * kesilir. Neden ana faturaya ek satır değil de ayrı fatura: aylık müşteriler bölüşümlü
     * (Marmara 2, CANTAŞ 3 cari) — ekstranın hangi parçaya gireceği belirsiz kalırdı. Ayrı
     * fatura müşterinin ANA carisine kesilir, muhasebede de ayrı görünür.
     *
     * Mükerrer kalkanı: kesilen kalemler `fatura_log_id` ile bu faturaya bağlanır (claim);
     * bağlanan kalem bir daha aday listesine girmez.
     * @param array{onay?:string,actor?:string} $ctx
     */
    public function createExtraInvoice(int $customerId, string $ay, array $ctx): array
    {
        if (!self::faturaAktif()) {
        ...
        // Aylık kalemler ay KAPANMADAN kesilmez (fable-056 kuralı burada da geçerli).
        if (Helpers::today() < $son) {
            return self::faturaSonuc(false, 'ay_kapanmadi', 'Ekstra kalemler ay kapanınca kesilir — '
                . date('d.m.Y', strtotime($son)) . ' tarihinde açılır.');
        }
        $contactId = trim((string) ($c['parasut_id'] ?? ''));
        if ($contactId === '') {
            return self::faturaSonuc(false, 'eslesme_yok', 'Paraşüt cari eşleşmesi yok — ekstra faturası kesilemez.');
        }

        $ham = $this->repo->ekstraKalemler($customerId, $ay, true);   // yalnız faturalanmamışlar
        if (!$ham) {
            return self::faturaSonuc(false, 'kalem_yok', 'Bu ay için faturalanmamış ekstra kalem yok.');
        }
        $varsayilanUrun = trim((string) $this->repo->ayar('ekstra_urun_id', ''));
        $kalemler = [];
        $ids = [];
        $logKalemler = [];
        $net = 0.0;
        foreach ($ham as $k) {
            $urun = trim((string) ($k['parasut_product_id'] ?? '')) !== ''
                ? trim((string) $k['parasut_product_id']) : $varsayilanUrun;
            if ($urun === '') {
                // Ürünsüz kalem Paraşüt'te 422 verir. SESSİZ DÜŞÜRME YOK — kesimi hiç yapma.
                return self::faturaSonuc(false, 'urun_yok', 'Ekstra ürün id tanımlı değil ("'
                    . $k['ad'] . '") — Fatura ekranından ekstra ürün id girin, sonra kesilir.');
            }
            $adet = (float) $k['adet'];
            $birim = (float) $k['birim_fiyat'];
            $kdv = (float) $k['kdv_orani'];
            $kalemler[] = ['ad' => (string) $k['ad'], 'adet' => $adet, 'birim' => $birim,
                'kdv' => $kdv, 'urun_id' => $urun];
            $ids[] = (int) $k['id'];
            $logKalemler[] = ['ogun' => 'ekstra', 'urun_id' => $urun, 'miktar' => $adet, 'birim_fiyat' => $birim];
            $net += round($adet * $birim, 2) * (1 + $kdv / 100);
        }
        $net = round($net, 2);
        $actor = (string) ($ctx['actor'] ?? '');
        $vadeGun = (int) ($c['fatura_vade_gun'] ?? 1);

        // ── CLAIM FIRST: log kaydını 'bilinmiyor' aç + kalemleri ona bağla ──
        $faturaLogId = $this->repo->faturaLogEkle([
            'customer_id' => $customerId, 'donem_bas' => $bas, 'donem_son' => $son, 'tip' => 'ekstra',
            'parasut_contact_id' => $contactId, 'alt_ad' => 'Ekstra kalemler',
            'kalemler' => $logKalemler, 'toplam_kisi' => count($kalemler), 'toplam_tutar' => $net,
            'durum' => 'bilinmiyor', 'entered_by' => $actor,
        ]);
        $claimed = $this->repo->ekstraKalemleriFaturayaBagla($ids, $faturaLogId);
        if ($claimed !== count($ids)) {
            $this->repo->faturaLogGuncelle($faturaLogId, ['durum' => 'hata',
                'hata_mesaj' => 'Çakışma — ekstra kalemler eşzamanlı başka faturaya bağlandı; kesim yapılmadı.']);
            return self::faturaSonuc(false, 'cakisma',
                'Ekstra kalemler eşzamanlı başka bir fatura tarafından alındı — baştan deneyin.');
        }

        $govde = $this->ekstraGovde($contactId, $son, $kalemler, $vadeGun);
        $r = $this->cagir('POST', '/sales_invoices?include=details', $govde);
        if ($r['net'] === 'connect') {
            $r = $this->cagir('POST', '/sales_invoices?include=details', $govde);
        }
        if ($r['net'] === 'timeout') {
            // Belge kesilmiş OLABİLİR → 'bilinmiyor' KALIR, kalemler bağlı kalır (tekrar kesilmesin).
            ...
            return self::faturaSonuc(false, 'bilinmiyor',
                'Ekstra: zaman aşımı — fatura kesilmiş olabilir. TEKRAR DENEMEYİN, Paraşüt\'ten kontrol edin.',
                null, null, 'yok', 'yok', $net, count($kalemler));
        }
        if ($r['net'] !== 'ok' || $r['status'] < 200 || $r['status'] >= 300) {
            $mesaj = 'Ekstra: Paraşüt reddetti (HTTP ' . $r['status'] . ')' . self::apiHata($r['data']);
            // 4xx = belge YOK → kalemleri geri aç, bir dahaki turda tekrar denensin.
            $this->repo->faturaLogGuncelle($faturaLogId, ['durum' => 'hata', 'hata_mesaj' => $mesaj]);
            $this->repo->ekstraKalemleriFaturadanCoz($ids, $faturaLogId);
            return self::faturaSonuc(false, 'hata', $mesaj, null, null, 'yok', 'yok', $net, count($kalemler));
        }
        ...
        uysa_audit('parasut_fatura', $actor, $son, json_encode([
            'customer_id' => $customerId, 'tip' => 'ekstra', 'kalem_sayisi' => count($kalemler),
            'durum' => 'kesildi', 'fatura_log_id' => $faturaLogId, 'fatura_no' => $faturaNo,
        ], JSON_UNESCAPED_UNICODE), '');
        return self::faturaSonuc(true, 'kesildi', count($kalemler) . ' ekstra kalem faturalandı.',
            $faturaId, $faturaNo, $resm, $mail, $net, count($kalemler));
    }
